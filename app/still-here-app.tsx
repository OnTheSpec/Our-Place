...
const REFLECTIONS = [
  "It sounds like the sunshine has left you feeling a little lighter today.",
  "Watering the tomatoes brought Frank and the blue porch swing close to mind.",
  "You’d like your family to know that milk is running low and the kitchen light needs a hand.",
];
...
function Header({ familyMode, view, go }: { familyMode: boolean; view: View; go: (v: View) => void }) {
  return <header className="topbar">
    <button className="brand" onClick={() => go(familyMode ? "family" : "home")} aria-label="Still Here home"><span className="brand-mark" aria-hidden="true">S</span><span>Still Here</span></button>
...
function Home({ onBegin, reply, routine, setRoutine, onInvite }: { onBegin: () => void; reply: Reply; routine: boolean; setRoutine: (v: boolean) => void; onInvite: () => void }) {
  return <section className="home page">
    <div className="welcome-copy">
      <span className="eyebrow">A quiet place to be heard</span>
...
function CheckIn({ step, listening, answers, textMode, textarea, setTextMode, setAnswers, capture, next, onExit }: CheckProps) {
  ...
  return <section className="checkin page">
    ...
    <div aria-live="polite">{answer && <div className="heard reflection"><span>♡</span><p><small>Let me see if I understand</small><strong>{REFLECTIONS[step]}</strong><em>Does that feel right?</em></p></div>}</div>
    <div className="single-choice">{answer ? <div className="reflection-actions"><button className="primary" onClick={next}>Yes, that’s right →</button><button className="secondary" onClick={() => { setAnswers(current => current.map((value, index) => index === step ? "" : value)); setTextMode(false); }}>Let me say it differently</button></div> : <button className="text-link" onClick={() => setTextMode(!textMode)}>{textMode ? "Use my voice instead" : "I’d rather type"}</button>}</div>
...
function Family({ reaction, setReaction, commitments, setCommitments, setReply, onMemories }: { reaction: string; setReaction: (v: string) => void; commitments: Record<string,string>; setCommitments: React.Dispatch<React.SetStateAction<Record<string,string>>>; setReply: (v: Reply) => void; onMemories: () => void }) {
  ...
  return <section className="family page">
    ...
    <p className="family-disclaimer">Still Here helps families connect. It never diagnoses or replaces professional or emergency care.</p>
  </section>;
}
